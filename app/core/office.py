...
Команды (в том числе прогоны тестов) запускай только в foreground и дожидайся их результата:
фоновый запуск (run_in_background) недопустим — уведомление о его завершении к тебе не придёт,
и задача закроется без результата. Долгий прогон разбивай на части (по файлу/тесту).
Когда закончишь — сделай `git add -A && git commit -m "<что сделал>"` и ответь резюме
с фактическим итогом проверок (что прогнал, сколько прошло/упало, что осталось).
"""
